...
**Distribution (v0.1):** Python on PyPI (`pip install sqube-agent-guard`), Node on npm (`npm install sqube-agent-guard`). The Rust crate lives under `rust/` in the GitHub repository — build with `cargo build` / `cargo test` after clone; **crates.io publish is not available yet** (do not document `cargo add sqube-agent-guard` as the primary install path until published).
